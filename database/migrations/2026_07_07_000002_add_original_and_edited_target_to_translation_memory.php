<?php
	...
	return new class extends Migration {
		public function up(): void
		{
			Schema::table('user_books_translation_memory', function (Blueprint $table) {
				if (!Schema::hasColumn('user_books_translation_memory', 'original_target_sentence')) {
					$table->text('original_target_sentence')->nullable()->after('source_sentence');
				}
				if (!Schema::hasColumn('user_books_translation_memory', 'edited_target_sentence')) {
					$table->text('edited_target_sentence')->nullable()->after('original_target_sentence');
				}
			});
		...
		public function down(): void
		{
			...
			Schema::table('user_books_translation_memory', function (Blueprint $table) {
				if (Schema::hasColumn('user_books_translation_memory', 'edited_target_sentence')) {
					$table->dropColumn('edited_target_sentence');
				}
				if (Schema::hasColumn('user_books_translation_memory', 'original_target_sentence')) {
					$table->dropColumn('original_target_sentence');
				}
			});
		}
	};
