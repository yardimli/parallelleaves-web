(() => {
  ...
  var Fragment = class _Fragment {
    ...
    */
    nodesBetween(from2, to, f, nodeStart = 0, parent2) {
      for (let i = 0, pos = 0; pos < to; i++) {
        let child = this.content[i], end = pos + child.nodeSize;
        if (end > from2 && f(child, nodeStart + pos, parent2 || null, i) !== false && child.content.size) {
          let start = pos + 1;
  ...
  function insertInto(content, dist, insert, parent2) {
    ...
    if (offset == dist || child.isText) {
      if (parent2 && !parent2.canReplace(index, index, insert))
        return null;
  ...
  function replaceOuter($from, $to, slice2, depth) {
    ...
    if (index == $to.index(depth) && depth < $from.depth - slice2.openStart) {
      ...
      return close(parent2, content.cut(0, $from.parentOffset).append(slice2.content).append(content.cut($to.parentOffset)));
    } else {
  ...
  function prepareSliceForReplace(slice2, $along) {
    let extra = $along.depth - slice2.openStart, parent2 = $along.node(extra);
    let node = parent2.copy(slice2.content);
  ...
  var ResolvedPos = class _ResolvedPos {
    ...
    */
    get nodeAfter() {
      let parent2 = this.parent, index = this.index(this.depth);
      if (index == parent2.childCount)
        return null;
      let dOff = this.pos - this.path[this.path.length - 1], child = parent2.child(index);
      return dOff ? parent2.child(index).cut(dOff) : child;
    }
    /**
    ...
    */
    marks() {
      let parent2 = this.parent, index = this.index();
      if (parent2.content.size == 0)
        return Mark.none;
      if (this.textOffset)
        return parent2.child(index).marks;
      let main = parent2.maybeChild(index - 1), other = parent2.maybeChild(index);
      if (!main) {
        let tmp = main;
        main = other;
        other = tmp;
  ...
  var ParseContext = class {
    ...
    addAll(parent2, marks2, startIndex, endIndex) {
      let index = startIndex || 0;
      for (let dom = startIndex ? parent2.childNodes[startIndex] : parent2.firstChild, end = endIndex == null ? null : parent2.childNodes[endIndex]; dom != end; dom = dom.nextSibling, ++index) {
        this.findAtPoint(parent2, index);
    ...
    findAtPoint(parent2, offset) {
      if (this.find)
        for (let i = 0; i < this.find.length; i++) {
          if (this.find[i].node == parent2 && this.find[i].offset == offset)
  ...
  var AddMarkStep = class _AddMarkStep extends Step {
    ...
    */
    ...
    apply(doc3) {
      ...
      let slice2 = new Slice(mapFragment(oldSlice.content, (node, parent3) => {
  ...
  function addMark(tr, from2, to, mark) {
    let removed = [], added = [];
    let removing, adding;
    tr.doc.nodesBetween(from2, to, (node, pos, parent2) => {
      if (!node.isInline)
        return;
      let marks2 = node.marks;
      if (!mark.isInSet(marks2) && parent2.type.allowsMarkType(mark.type)) {
  ...
  function dropPoint(doc3, pos, slice2) {
    ...
    for (let pass = 1; pass <= (slice2.openStart == 0 && slice2.size ? 2 : 1); pass++) {
      for (let d = $pos.depth; d >= 0; d--) {
        ...
        let parent2 = $pos.node(d), fits = false;
        if (pass == 1) {
      ...
      for (let pass = 1; pass <= 2; pass++) {
        for (let sliceDepth = pass == 1 ? startDepth : this.unplaced.openStart; sliceDepth >= 0; sliceDepth--) {
          ...
          for (let frontierDepth = this.depth; frontierDepth >= 0; frontierDepth--) {
            ...
            if (pass == 1 && (first ? match.matchType(first.type) || (inject = match.fillBefore(Fragment.from(first), false)) : parent2 && type.compatibleContent(parent2.type)))
  ...
  function replaceRange(tr, from2, to, slice2) {
    ...
    for (let j = slice2.openStart; j >= 0; j--) {
      ...
      for (let i = 0; i < targetDepths.length; i++) {
        ...
        let parent2 = $from.node(targetDepth - 1), index = $from.index(targetDepth - 1);
        if (parent2.canReplaceWith(index, index, insert.type, insert.marks))
  ...
  function closeFragment(fragment, depth, oldOpen, newOpen, parent2) {
    ...
    if (depth > newOpen) {
      let match = parent2.contentMatchAt(0);
    ...
    */
    applyInner(tr) {
      ...
      for (let i = 0; i < fields.length; i++) {
        let field2 = fields[i];
        newInstance[field2.name] = field2.apply(tr, this[field2.name], this, newInstance);
      }
      return newInstance;
    }
    /**
          ...
          if (pluginFields)
            for (let prop in pluginFields) {
              ...
              if (plugin.key == field2.name && state && state.fromJSON && Object.prototype.hasOwnProperty.call(json, prop)) {
                instance[field2.name] = state.fromJSON.call(plugin, config, json[prop], instance);
                return;
              }
            }
  ...
  var parentNode = function(node) {
    let parent2 = node.assignedSlot || node.parentNode;
  ...
  function scrollRectIntoView(view, rect, startDOM) {
    ...
    for (let parent2 = startDOM || view.dom; ; ) {
      if (!parent2)
        break;
      if (parent2.nodeType != 1) {
        parent2 = parentNode(parent2);
        continue;
      }
      let elt = parent2;
      ...
      let pos = atTop ? "fixed" : getComputedStyle(parent2).position;
      if (/^(fixed|sticky)$/.test(pos))
        break;
      parent2 = pos == "absolute" ? parent2.offsetParent : parentNode(parent2);
    }
  }
  ...
  var MarkViewDesc = class _MarkViewDesc extends ViewDesc {
    ...
    static create(parent2, mark, inline, view) {
      let custom = view.nodeViews[mark.type.name];
      let spec = custom && custom(mark, view, inline);
      if (!spec || !spec.dom)
        spec = DOMSerializer.renderSpec(document, mark.type.spec.toDOM(mark, inline), null, mark.attrs);
      return new _MarkViewDesc(parent2, mark, spec.dom, spec.contentDOM || spec.dom, spec);
    }
    parseRule() {
  ...
  var NodeViewDesc = class _NodeViewDesc extends ViewDesc {
    constructor(parent2, node, outerDeco, innerDeco, dom, contentDOM, nodeDOM, view, pos) {
      super(parent2, [], dom, contentDOM);
      this.node = node;
    ...
    // never need.)
    static create(parent2, node, outerDeco, innerDeco, view, pos) {
      ...
      if (spec)
        ...
        return new _NodeViewDesc(parent2, node, outerDeco, innerDeco, dom, contentDOM || null, nodeDOM, view, pos + 1);
    }
    parseRule() {
  ...
  var TextViewDesc = class _TextViewDesc extends NodeViewDesc {
    constructor(parent2, node, outerDeco, innerDeco, dom, nodeDOM, view) {
      super(parent2, node, outerDeco, innerDeco, dom, null, nodeDOM, view, 0);
    }
    parseRule() {
  ...
  var CustomNodeViewDesc = class extends NodeViewDesc {
    constructor(parent2, node, outerDeco, innerDeco, dom, contentDOM, nodeDOM, spec, view, pos) {
      super(parent2, node, outerDeco, innerDeco, dom, contentDOM, nodeDOM, view, pos);
      this.spec = spec;
    }
  ...
  function iterDeco(parent2, deco, onWidget, onNode) {
    let locals = deco.locals(parent2), offset = 0;
    if (locals.length == 0) {
  ...
  function normalizeSiblings(fragment, $context) {
    if (fragment.childCount < 2)
      return fragment;
    for (let d = $context.depth; d >= 0; d--) {
      let parent2 = $context.node(d);
      let match = parent2.contentMatchAt($context.index(d));
      let lastWrap, result = [];
      fragment.forEach((node) => {
        if (!result)
          return;
  ...
  function fixUpBadSafariComposition(view, addedNodes) {
    ...
    for (let node of addedNodes) {
      if (((_a = node.parentNode) === null || _a === void 0 ? void 0 : _a.nodeName) == "TR") {
        ...
        if (nextCell) {
          let parent2 = nextCell;
          for (; ; ) {
            let first = parent2.firstChild;
  ...
  function parseBetween(view, from_, to_) {
    let { node: parent2, fromOffset, toOffset, from: from2, to } = view.docView.parseRange(from_, to_);
    let domSel = view.domSelectionRange();
    let find;
    ...
    if (chrome && view.input.lastKeyCode === 8) {
      for (let off = toOffset; off > fromOffset; off--) {
        let node = parent2.childNodes[off - 1], desc = node.pmViewDesc;
    ...
    let sel = null, doc3 = parser.parse(parent2, {
      topNode: $from.parent,
      ...
      if (dispatch) {
        if ($cursor) {
          ...
          if (removeWhenPresent) {
            ...
            add2 = !ranges.every((r) => {
              let missing = false;
              tr.doc.nodesBetween(r.$from.pos, r.$to.pos, (node, pos, parent2) => {
                if (missing)
                  return false;
                missing = !markType.isInSet(node.marks) && !!parent2 && parent2.type.allowsMarkType(markType) && !(node.isText && /^\s*$/.test(node.textBetween(Math.max(0, r.$from.pos - pos), Math.min(node.nodeSize, r.$to.pos - pos))));
              });
              return !missing;
            });
          }
  ...
  function liftOutOfList(state, dispatch, range) {
    ...
    if (!parent2.canReplace(indexBefore + (atStart ? 0 : 1), indexBefore + 1, item.content.append(atEnd ? Fragment.empty : Fragment.from(list))))
      return false;
  ...
  // src/js/book-planner/editor-shortcuts.js
  function createShortcutKeymap(postToParent2) {
    return keymap({
      "Mod-f": () => {
        postToParent2("shortcut:find");
        return true;
      },
      "Mod-h": () => {
        postToParent2("shortcut:find-replace");
        return true;
      },
      "Mod-1": () => {
        postToParent2("shortcut:focus-source");
        return true;
      },
      "Mod-2": () => {
        postToParent2("shortcut:focus-target");
        return true;
      },
      "Mod-t": (state, dispatch, view) => {
        ...
        if (floatingBtn) {
          ...
          if (selection.empty) {
            ...
            const pos = $from.pos;
            postToParent2("requestTranslation", { from: pos, to: pos });
            floatingBtn.remove();
          }
        }
        return true;
  ...
  // src/js/book-planner/editor-iframe-main.js
  ...
  var chapterId;
  var field;
  var hasSourceSelection = false;
  var floatingTranslateBtn = null;
  var localSearchMatches = [];
  var localSearchReplaceMatches = [];
  ...
  var searchReplacePlugin = new Plugin({
    state: {
      init() {
        return DecorationSet.empty;
      },
  ...
  var postToParent = (type, payload) => {
    if (!parentOrigin || !parent.window) return;
    parent.window.postMessage({ type, payload }, parentOrigin);
  };
  var debouncedLogEdit = debounce(() => {
    if (!editorView) return;
    const { doc: doc3, selection } = editorView.state;
    ...
    if (lastOpening && firstClosing) {
      const contentFragment = doc3.slice(lastOpening.to, firstClosing.from);
  ...
  var getToolbarState = (state) => {
    const { $from, from: from2, to, empty: empty2 } = state.selection;
    const { schema: schema3 } = state;
    const isMarkActive = (type) => {
      if (!type) return false;
      if (empty2) return !!(state.storedMarks || $from.marks()).some((mark) => mark.type === type);
    ...
    let headingLevel = 0;
    if (parent2.type.name === "heading") {
      headingLevel = parent2.attrs.level;
    }
    return {
      canUndo: undo(state),
      canRedo: redo(state),
      isTextSelected: !empty2,
      activeMarks: Object.keys(schema3.marks).filter((markName) => isMarkActive(schema3.marks[markName])),
      activeNodes: Object.keys(schema3.nodes).filter((nodeName) => isNodeActive(schema3.nodes[nodeName])),
      headingLevel,
      selectionText: state.doc.textBetween(from2, to, " ")
    };
  };
  function manageFloatingButton(view) {
    ...
    const { $from } = selection;
    const parentNode2 = $from.parent;
    if (parentNode2.type.name === "paragraph" && parentNode2.content.size === 0) {
      const pos = $from.pos;
      ...
      floatingTranslateBtn.title = "Translate selected source text here";
  ...
  function createEditorView(mount, config) {
    ...
    const editorPlugin = new Plugin({
      props: {
        ...
        handleClick(view, pos, event) {
          ...
          const parentNode2 = resolvedPos.parent;
          const offsetInParent = resolvedPos.parentOffset;
          if (parentNode2.isTextblock) {
            const textContent = parentNode2.textContent;
            const markerRegex = /(\[\[#(\d+)\]\])|(\{\{#(\d+)\}\})/g;
            let match;
    ...
    const doc3 = DOMParser.fromSchema(schema2).parse(document.createRange().createContextualFragment(initialHtml || ""));
  ...
  function executeCommand({ command, attrs }) {
    ...
    switch (command) {
      ...
      case "strike":
        ...
        break;
      case "blockquote":
        cmd = state.selection.$from.depth > 1 && state.selection.$from.node(-1).type === schema3.nodes.blockquote ? lift2 : wrapIn(schema3.nodes.blockquote);
        break;
      case "bullet_list":
        cmd = liftListItem(schema3.nodes.list_item)(state) ? liftListItem(schema3.nodes.list_item) : wrapInList(schema3.nodes.bullet_list);
        break;
      case "ordered_list":
        cmd = liftListItem(schema3.nodes.list_item)(state) ? liftListItem(schema3.nodes.list_item) : wrapInList(schema3.nodes.ordered_list);
        break;
      case "horizontal_rule":
        dispatch(state.tr.replaceSelectionWith(schema3.nodes.horizontal_rule.create()));
        break;
      case "heading":
        cmd = attrs.level === 0 ? setBlockType2(schema3.nodes.paragraph) : setBlockType2(schema3.nodes.heading, { level: attrs.level });
        break;
      case "highlight":
        let tr = state.tr;
        const { from: from2, to } = state.selection;
        Object.keys(schema3.marks).forEach((markName) => {
          if (markName.startsWith("highlight_")) tr = tr.removeMark(from2, to, schema3.marks[markName]);
        });
        if (attrs.color !== "transparent") {
          const markType = schema3.marks[`highlight_${attrs.color}`];
          if (markType) tr = tr.addMark(from2, to, markType.create());
        }
        dispatch(tr);
        break;
    }
  ...
  function performSearch(query) {
    ...
    if (!query) return;
    const { doc: doc3 } = editorView.state;
    const decorations = [];
    ...
    doc3.descendants((node, pos) => {
      ...
      while ((match = regex.exec(node.text)) !== null) {
        ...
        localSearchMatches.push({ from: from2, to });
        decorations.push(Decoration.inline(from2, to, { class: "search-highlight" }));
      }
    });
  ...
  function navigateToSearchMatch(matchIndex, isActive) {
    ...
    const decorations = localSearchMatches.map((match, i) => {
      const className = isActive && i === matchIndex ? "search-highlight search-highlight-active" : "search-highlight";
  ...
  window.addEventListener("message", (event) => {
    ...
    switch (type) {
      ...
      case "sourceSelectionChanged":
        ...
        break;
      case "replaceRange": {
        const { from: from2, to, newContentHtml } = payload;
        const { state, dispatch } = editorView;
        const { schema: schema3 } = state;
        const tempDiv = document.createElement("div");
        tempDiv.innerHTML = newContentHtml;
        const newFragment = DOMParser.fromSchema(schema3).parseSlice(tempDiv).content;
        let tr = state.tr.replaceWith(from2, to, newFragment);
        let finalTo = from2 + newFragment.size;
        const mark = schema3.marks.ai_suggestion.create();
        tr = tr.addMark(from2, finalTo, mark);
        dispatch(tr);
        const currentState = editorView.state;
        const $replaceStart = currentState.doc.resolve(from2);
        const nodeBefore = $replaceStart.parent;
        if (nodeBefore && nodeBefore.type.name === "paragraph" && nodeBefore.content.size === 0) {
          const paraFrom = from2 - nodeBefore.nodeSize;
          const paraTo = from2;
        ...
        const finalRange = { from: from2, to: finalTo };
        const endCoords = editorView.coordsAtPos(finalTo);
        postToParent("replacementComplete", { finalRange, endCoords });
        break;
      }
      case "findAndScrollToText": {
        ...
        if (foundPos !== -1) {
          const tr = state.tr.setSelection(TextSelection.create(doc3, foundPos, foundPos + text.length));
          editorView.dispatch(tr);
          editorView.focus();
          const { from: from2 } = editorView.state.selection;
          const coords = editorView.coordsAtPos(from2);
          postToParent("scrollToCoordinates", { top: coords.top });
        }
        break;
      ...
      case "search-replace:find": {
        ...
        doc3.descendants((node, pos) => {
          ...
          while ((match = regex.exec(node.text)) !== null) {
            const from2 = pos + match.index;
            const to = from2 + match[0].length;
            localSearchReplaceMatches.push({ from: from2, to });
            decorations.push(Decoration.inline(from2, to, { class: "search-replace-highlight" }));
          }
        });
        const decorationSet = DecorationSet.create(doc3, decorations);
      ...
      case "search-replace:navigateTo": {
        ...
        const decorations = localSearchReplaceMatches.map((match, i) => {
          const className = isActive && i === matchIndex ? "search-replace-highlight search-replace-highlight-active" : "search-replace-highlight";
          return Decoration.inline(match.from, match.to, { class: className });
        });
        const decorationSet = DecorationSet.create(editorView.state.doc, decorations);
        const tr = editorView.state.tr.setMeta("search-replace", { decorations: decorationSet });
        ...
        if (isActive && localSearchReplaceMatches[matchIndex]) {
          const { from: from2 } = localSearchReplaceMatches[matchIndex];
      ...
      case "search-replace:replace": {
        ...
        const tr = editorView.state.tr.replaceWith(from2, to, editorView.state.schema.text(replaceText));
        editorView.dispatch(tr);
      ...
      case "search-replace:replaceAll": {
        ...
        editorView.state.doc.descendants((node, pos) => {
          ...
          for (let i = matchesInNode.length - 1; i >= 0; i--) {
            const { from: from2, to } = matchesInNode[i];
            const mapping = tr.mapping;
            tr.replaceWith(mapping.map(from2), mapping.map(to), editorView.state.schema.text(replaceText));
            replacementsMade++;
          }
          return true;
        });
      ...
      case "cleanupAiSuggestion": {
        ...
        tr.removeMark(0, editorView.state.doc.content.size, schema2.marks.ai_suggestion);
        editorView.dispatch(tr);
        editorView.focus();
        break;
      }
      case "discardAiSuggestion": {
        const { from: from2, to, originalFragmentJson } = payload;
        const originalFragment = Fragment.fromJSON(schema2, originalFragmentJson);
        let tr = editorView.state.tr.replaceWith(from2, to, originalFragment);
        const newTo = from2 + originalFragment.size;
        tr = tr.setSelection(TextSelection.create(tr.doc, from2, newTo));
        editorView.dispatch(tr);
        break;
      }
      case "setSelection": {
        const { from: from2, to } = payload;
        const tr = editorView.state.tr.setSelection(TextSelection.create(editorView.state.doc, from2, to));
        editorView.dispatch(tr);
        break;
      }
      case "prepareForRephrase": {
        ...
        const textBefore = doc3.textBetween(0, from2, " ");
        const wordsBefore = textBefore.trim().split(/\s+/).slice(-200).join(" ");
        const textAfter = doc3.textBetween(to, doc3.content.size, " ");
        const wordsAfter = textAfter.trim().split(/\s+/).slice(0, 200).join(" ");
        postToParent("selectionResponse", {
          from: from2,
          to,
          originalFragmentJson: doc3.slice(from2, to).content.toJSON(),
          selectedText: doc3.textBetween(from2, to, " "),
          wordsBefore,
          wordsAfter
        });
        break;
      ...
      case "getSelectionText": {
        ...
        const { selection, doc: doc3 } = editorView.state;
        const selectedText = selection.empty ? "" : doc3.textBetween(selection.from, selection.to, " ");
