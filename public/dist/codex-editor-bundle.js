(() => {
  ...
  // src/js/novel-planner/codex-entry-main.js
  ...
  var sourceEditorView = null;
  var targetEditorView = null;
  ...
  var currentEditorState = null;
  var placeholderPlugin = (placeholderText) => new Plugin({
    props: {
      decorations(state) {
        const { doc: doc3 } = state;
        if (doc3.childCount === 1 && doc3.firstChild.isTextblock && doc3.firstChild.content.size === 0) {
          return DecorationSet.create(doc3, [
            Decoration.node(0, doc3.firstChild.nodeSize, {
  ...
  function setupContentEditor(mount, options = {}) {
    ...
    const doc3 = DOMParser.fromSchema(schema2).parse(initialContent);
  ...
  function updateCodexToolbarState(view) {
    ...
    if (currentEditorState) {
      allBtns.forEach((btn) => {
        ...
        switch (cmd) {
          case "undo":
            btn.disabled = !currentEditorState.canUndo;
            break;
          case "redo":
            btn.disabled = !currentEditorState.canRedo;
            break;
          case "bold":
            btn.classList.toggle("active", currentEditorState.activeMarks.includes("strong"));
            break;
          case "italic":
            btn.classList.toggle("active", currentEditorState.activeMarks.includes("em"));
            break;
          case "underline":
            btn.classList.toggle("active", currentEditorState.activeMarks.includes("underline"));
            break;
          case "strike":
            btn.classList.toggle("active", currentEditorState.activeMarks.includes("strike"));
            break;
          case "blockquote":
            btn.classList.toggle("active", currentEditorState.activeNodes.includes("blockquote"));
            break;
          case "bullet_list":
            btn.classList.toggle("active", currentEditorState.activeNodes.includes("bullet_list"));
            break;
          case "ordered_list":
            btn.classList.toggle("active", currentEditorState.activeNodes.includes("ordered_list"));
            break;
        }
  ...
  async function handleToolbarAction(button) {
    ...
    const schema3 = view.state.schema;
    let cmdFunc = null;
    switch (command) {
      case "undo":
        cmdFunc = undo;
      ...
      case "redo":
        ...
        break;
      case "bold":
        cmdFunc = toggleMark(schema3.marks.strong);
        break;
      case "italic":
        cmdFunc = toggleMark(schema3.marks.em);
        break;
      case "underline":
        cmdFunc = toggleMark(schema3.marks.underline);
        break;
      case "strike":
        cmdFunc = toggleMark(schema3.marks.strike);
        break;
      case "blockquote":
        cmdFunc = wrapIn(schema3.nodes.blockquote);
        break;
      case "bullet_list":
        cmdFunc = wrapInList(schema3.nodes.bullet_list);
        break;
      case "ordered_list":
        cmdFunc = wrapInList(schema3.nodes.ordered_list);
        break;
    ...
    if (button.classList.contains("js-heading-option")) {
      ...
      cmdFunc = level === 0 ? setBlockType2(schema3.nodes.paragraph) : setBlockType2(schema3.nodes.heading, { level });
      if (document.activeElement) document.activeElement.blur();
    }
  ...
  async function setupCreateMode(novelId, selectedText) {
    ...
    activeEditorView = sourceEditorView;
    setupCodexToolbar();
    ...
    try {
      ...
      titleInput.value = selectedText.slice(0, 50);
    } finally {
  ...
  async function setupEditMode(entryId) {
    ...
    try {
      ...
      activeEditorView = sourceEditorView;
      setupCodexToolbar();
      updateCodexToolbarState(activeEditorView);
      const titleInput = document.getElementById("js-codex-title-input");
      const phrasesInput = document.getElementById("js-codex-phrases-input");
