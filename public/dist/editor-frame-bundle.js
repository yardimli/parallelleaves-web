(() => {
  ...
  // src/js/novel-planner/content-editor.js
  ...
  var nodesWithNote = nodes2.addBefore("horizontal_rule", "note", noteNodeSpec);
  ...
  var NoteNodeView = class {
    ...
    openEditModal(e) {
      ...
      if (window.self !== window.top) {
        ...
        if (noteModal) {
          noteModal.querySelector(".js-note-modal-title").textContent = "Edit Note";
          document.getElementById("note-content-input").value = this.node.attrs.text;
          document.getElementById("note-pos").value = this.getPos();
          noteModal.showModal();
        }
      }
    }
  ...
  // src/js/novel-planner/editor-frame.js
  var view;
  var chapterId;
  var saveField;
  var isEditable;
  ...
  var debouncedContentSave = debounce(() => {
    ...
    window.parent.postMessage({
      type: "save-content",
      payload: { chapterId, field: saveField, value }
    }, "*");
  }, 2e3);
  function getToolbarState(state) {
    const { schema: schema3, selection } = state;
    const { $from, empty: empty2 } = selection;
    const isMarkActive = (type) => {
      if (!type) return false;
      const { from: from2, to, empty: empty3 } = state.selection;
      if (empty3) return !!(state.storedMarks || $from.marks()).some((mark) => mark.type === type);
      return state.doc.rangeHasMark(from2, to, type);
    };
    const isNodeActive2 = (type) => {
      for (let i = $from.depth; i > 0; i--) {
    ...
    return {
      canUndo: undo(state),
      canRedo: redo(state),
      bold: isMarkActive(schema3.marks.strong),
      italic: isMarkActive(schema3.marks.em),
      underline: isMarkActive(schema3.marks.underline),
      strike: isMarkActive(schema3.marks.strike),
      blockquote: isNodeActive2(schema3.nodes.blockquote),
      bullet_list: isNodeActive2(schema3.nodes.bullet_list),
      ordered_list: isNodeActive2(schema3.nodes.ordered_list),
      headingLevel,
      canAddNote: empty2 && $from.parent.type.name === "paragraph" && $from.parent.content.size === 0,
      isTextSelected: !empty2,
      selectionText: !empty2 ? state.doc.textBetween(selection.from, selection.to, " ") : ""
    };
  }
  function executeCommand(command, attrs = {}) {
    ...
    switch (command) {
      ...
      case "heading":
        ...
        break;
      case "add_note":
        const noteNode = schema3.nodes.note.create({ text: attrs.text });
        const { $from } = state.selection;
        const tr = state.tr.replaceRangeWith($from.start(), $from.end(), noteNode);
        dispatch(tr);
        break;
      case "update_note":
        const trUpdate = state.tr.setNodeMarkup(attrs.pos, null, { text: attrs.text });
        dispatch(trUpdate);
        break;
      case "highlight":
        let trHighlight = state.tr;
        Object.keys(schema3.marks).forEach((markName) => {
          if (markName.startsWith("highlight_")) {
            trHighlight = trHighlight.removeMark(state.selection.from, state.selection.to, schema3.marks[markName]);
          }
        });
        if (attrs.color !== "transparent") {
          const markType = schema3.marks[`highlight_${attrs.color}`];
          if (markType) {
            trHighlight = trHighlight.addMark(state.selection.from, state.selection.to, markType.create());
          }
        }
        dispatch(trHighlight);
        break;
    }
    if (cmd) {
      cmd(state, dispatch);
  ...
  function init(config) {
    const { initialHtml, theme } = config;
    ...
    view = new EditorView(document.getElementById("editor-mount"), {
      state: EditorState.create({
        doc: DOMParser.fromSchema(schema2).parse(document.createRange().createContextualFragment(initialHtml || "")),
        plugins: [
          history(),
          ...
          new Plugin({
            props: {
              ...
              handleDOMEvents: {
                focus: () => {
                  window.parent.postMessage({ type: "editor-focus", payload: { editorState: getToolbarState(view.state) } }, "*");
                  return false;
                },
                blur: () => {
                  window.parent.postMessage({ type: "editor-blur" }, "*");
                  return false;
                }
              }
            }
      ...
      nodeViews: {
        note(node, view2, getPos) {
          return new NoteNodeView(node, view2, getPos);
        }
      },
        ...
        if (transaction.docChanged || transaction.selectionSet) {
          if (this.hasFocus()) {
            window.parent.postMessage({
              ...
              payload: { editorState: getToolbarState(newState) }
            }, "*");
          }
        }
      }
  ...
  window.addEventListener("message", (event) => {
    const { type, payload, channel } = event.data;
    switch (type) {
      case "init":
        init(payload);
        break;
      case "set-theme":
        ...
        break;
      case "apply-command":
        executeCommand(payload.command, payload.attrs);
        break;
      case "get-selection-data":
        const { from: from2, to, empty: empty2 } = view.state.selection;
        const slice2 = !empty2 ? view.state.doc.slice(from2, to) : null;
        window.parent.postMessage({
          type: "selection-data-response",
          channel,
          payload: { from: from2, to, sliceJSON: slice2 ? slice2.toJSON() : null }
        }, "*");
        break;
      case "stream-update": {
        const { schema: schema3 } = view.state;
        const mark = schema3.marks.ai_suggestion.create();
        let tr = view.state.tr;
        if (payload.isFirstChunk) {
          tr.replaceWith(payload.range.from, payload.range.to, []);
        }
        const textNode = schema3.text(payload.chunk, [mark]);
        tr.insert(payload.insertPos, textNode);
        view.dispatch(tr);
        break;
      }
      case "stream-replace": {
        const { from: from3, to: to2 } = payload.range;
        const { sliceJSON } = payload;
        const slice3 = Slice.fromJSON(schema2, sliceJSON);
        view.dispatch(view.state.tr.replace(from3, to2, slice3));
        break;
      }
      case "stream-finalize": {
        const { from: from3, to: to2 } = payload.range;
        const { schema: schema3 } = view.state;
        let tr = view.state.tr.removeMark(from3, to2, schema3.marks.ai_suggestion);
        const deletions = [];
        view.state.doc.nodesBetween(from3, to2, (node, pos) => {
          if (pos >= from3 && node.type.name === "paragraph" && node.content.size === 0) {
        ...
        view.dispatch(tr);
        window.parent.postMessage({ type: "stream-finalized-ack", channel }, "*");
        break;
      }
      case "get-coords-at-pos":
        const coords = view.coordsAtPos(payload.pos);
        window.parent.postMessage({ type: "coords-at-pos-response", channel, payload: { coords } }, "*");
        break;
      case "set-selection":
        view.dispatch(view.state.tr.setSelection(TextSelection.create(view.state.doc, payload.from, payload.to)));
        break;
      case "focus":
        view.focus();
        break;
