(() => {
  ...
  var debouncedSave = debounce(async () => {
  ...
  function updateToolbarState(view) {
    if (!view) {
      ...
      currentEditorState = {
        ...
        activeMarks: Object.keys(schema2.marks).filter((markName) => isMarkActive(schema2.marks[markName])),
        activeNodes: Object.keys(schema2.nodes).filter((nodeName) => isNodeActive(schema2.nodes[nodeName])),
        headingLevel,
  ...
  function handleToolbarAction(button) {
    ...
    const schema2 = editorView.state.schema;
    let cmdFunc = null;
    switch (command) {
      case "undo":
        cmdFunc = undo;
      ...
      case "redo":
        ...
        break;
      case "bold":
        cmdFunc = toggleMark(schema2.marks.strong);
        break;
      case "italic":
        cmdFunc = toggleMark(schema2.marks.em);
        break;
      case "underline":
        cmdFunc = toggleMark(schema2.marks.underline);
        break;
      case "strike":
        cmdFunc = toggleMark(schema2.marks.strike);
        break;
      case "blockquote":
        cmdFunc = wrapIn(schema2.nodes.blockquote);
        break;
      case "bullet_list":
        cmdFunc = wrapInList(schema2.nodes.bullet_list);
        break;
      case "ordered_list":
        cmdFunc = wrapInList(schema2.nodes.ordered_list);
        break;
    }
    if (button.classList.contains("js-heading-option")) {
      const level = parseInt(button.dataset.level, 10);
      cmdFunc = level === 0 ? setBlockType2(schema2.nodes.paragraph) : setBlockType2(schema2.nodes.heading, { level });
  ...
  function setupEditor(mount, initialContent) {
    const doc3 = DOMParser.fromSchema(codexSchema).parse(document.createRange().createContextualFragment(initialContent));
  ...
  async function setupAutogenCodex(novelId2) {
    ...
    modalContent.addEventListener("submit", (event) => {
      ...
      window.api.startCodexAutogen({ novelId: novelId2, model, temperature });
    });
