...
/** Mount an independent ProseMirror chapter editor in the current document. */
export function createChapterEditor(mount, emit) {
	let editorView;
	let chapterId;
	let field;
	...
	/**
	 * Emits an editor event to the host page.
	 * @param {string} type - The message type.
	 * @param {object} [payload] - The message payload.
	 */
	const postToParent = (type, payload) => {
		if (type === 'scrollToCoordinates') payload = {...payload, top: payload.top - mount.getBoundingClientRect().top};
		emit(type, payload);
	};
	...
	function manageFloatingButton(view) {
		...
		if (parentNode.type.name === 'paragraph' && parentNode.content.size === 0) {
			...
			mount.appendChild(floatingTranslateBtn);

			// Position the button to the left of the cursor, vertically centered
			floatingTranslateBtn.style.position = 'absolute';
			floatingTranslateBtn.style.left = `${coords.left - mount.getBoundingClientRect().left + 10}px`;
			floatingTranslateBtn.style.top = `${coords.top - mount.getBoundingClientRect().top}px`;
	...
	/**
	 * Executes a formatting or editor command received from the host page.
	...
	// Applies typography styles received from the host page.
	...
	/**
	 * Main message listener for communication from the host page.
	 */
	function dispatch({type, payload}) {

		switch (type) {
			case 'init':
				createEditorView(mount, payload);
			...
			case 'replaceRange': {
				...
				const coords = editorView.coordsAtPos(finalTo);
				const mountRect = mount.getBoundingClientRect();
				const endCoords = {top: coords.top - mountRect.top, bottom: coords.bottom - mountRect.top, left: coords.left - mountRect.left, right: coords.right - mountRect.left};
				const suggestionEl = mount.querySelector('.ai-suggestion');
				...
				postToParent('replacementComplete', {
					...
					suggestionRect: suggestionRect ? {
						top: suggestionRect.top - mountRect.top,
						right: suggestionRect.right - mountRect.left,
						bottom: suggestionRect.bottom - mountRect.top,
						left: suggestionRect.left - mountRect.left,
	...
	return {dispatch, get view() { return editorView; }, destroy() {
		floatingTranslateBtn?.remove();
		editorView?.destroy();
	}};
}
